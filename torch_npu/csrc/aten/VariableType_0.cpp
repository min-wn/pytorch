...
// @generated from /vllm-workspace/pytorch/codegen/autograd/templates/VariableType.cpp
...
at::Tensor stft(c10::DispatchKeySet ks, const at::Tensor & self, int64_t n_fft, ::std::optional<int64_t> hop_length, ::std::optional<int64_t> win_length, const ::std::optional<at::Tensor> & window, bool normalized, ::std::optional<bool> onesided, ::std::optional<bool> return_complex, ::std::optional<bool> align_to_window) {
  ...
  auto _tmp = ([&]() {
    at::AutoDispatchBelowADInplaceOrView guard;
      return at::redispatch::stft(ks & c10::after_autograd_keyset, self_, n_fft, hop_length, win_length, window, normalized, onesided, return_complex, align_to_window);
  })();
  auto result = std::move(_tmp);
  #ifndef NDEBUG
  if (self__storage_saved.has_value() &&
...
at::Tensor inverse(c10::DispatchKeySet ks, const at::Tensor & self) {
  ...
  std::shared_ptr<InverseBackward0> grad_fn;
  if (_any_requires_grad) {
    grad_fn = std::shared_ptr<InverseBackward0>(new InverseBackward0(), deleteNode);
    grad_fn->set_next_edges(collect_next_edges( self ));
  }
  #ifndef NDEBUG
  auto self__storage_saved =
  ...
  if (!at::impl::dispatch_mode_enabled() && !at::impl::tensor_has_dispatch(result))
    TORCH_INTERNAL_ASSERT(result.use_count() <= 1, "function: inverse");
  #endif
  if (grad_fn) {
      set_history(flatten_tensor_args( result ), grad_fn);
  }
...
at::Tensor & inverse_out_out(c10::DispatchKeySet ks, const at::Tensor & self, at::Tensor & out) {
  ...
  TORCH_CHECK_NOT_IMPLEMENTED(!((isFwGradDefined(self) || isFwGradDefined(out))), "Trying to use forward AD with inverse_out that does not support it because it is an out= function");
  return out;
}
...
at::Tensor _npu_format_cast_aclnn(c10::DispatchKeySet ks, const at::Tensor & self, int64_t acl_format, int64_t customize_dtype) {
  ...
  std::shared_ptr<NpuFormatCastBackward1> grad_fn;
  if (_any_requires_grad) {
    grad_fn = std::shared_ptr<NpuFormatCastBackward1>(new NpuFormatCastBackward1(), deleteNode);
    grad_fn->set_next_edges(collect_next_edges( self ));
  }
  #ifndef NDEBUG
  auto self__storage_saved =
  ...
  auto _tmp = ([&]() {
    at::AutoDispatchBelowADInplaceOrView guard;
      return at_npu::redispatch::_npu_format_cast(ks & c10::after_autograd_keyset, self_, acl_format, customize_dtype);
  })();
  auto result = std::move(_tmp);
  #ifndef NDEBUG
  if (self__storage_saved.has_value() &&
  ...
  if (!at::impl::dispatch_mode_enabled() && !at::impl::tensor_has_dispatch(result))
    TORCH_INTERNAL_ASSERT(result.use_count() <= 1, "function: _npu_format_cast_aclnn");
  #endif
  if (grad_fn) {
      set_history(flatten_tensor_args( result ), grad_fn);
  }
...
::std::tuple<at::Tensor,at::Tensor,at::Tensor> npu_moe_token_permute_with_routing_map(c10::DispatchKeySet ks, const at::Tensor & tokens, const at::Tensor & routing_map, const ::std::optional<at::Tensor> & probs, ::std::optional<int64_t> num_out_tokens, bool drop_and_pad) {
  auto& tokens_ = unpack(tokens, "tokens", 0);
  auto& routing_map_ = unpack(routing_map, "routing_map", 1);
  [[maybe_unused]] auto _any_requires_grad = compute_requires_grad( tokens, probs );

  std::shared_ptr<NpuMoeTokenPermuteWithRoutingMapBackward0> grad_fn;
  if (_any_requires_grad) {
    grad_fn = std::shared_ptr<NpuMoeTokenPermuteWithRoutingMapBackward0>(new NpuMoeTokenPermuteWithRoutingMapBackward0(), deleteNode);
    grad_fn->set_next_edges(collect_next_edges( tokens, probs ));
    grad_fn->drop_and_pad = drop_and_pad;
    grad_fn->routing_map_ = SavedVariable(routing_map, false);
    grad_fn->routing_map_sym_argsize_1 = routing_map.sym_size(1);
    grad_fn->tokens_sym_argsize_0 = tokens.sym_size(0);
  }
  #ifndef NDEBUG
  auto tokens__storage_saved =
    tokens_.has_storage() ? ::std::optional<Storage>(tokens_.storage()) : ::std::nullopt;
  c10::intrusive_ptr<TensorImpl> tokens__impl_saved;
  if (tokens_.defined()) tokens__impl_saved = tokens_.getIntrusivePtr();
  auto routing_map__storage_saved =
    routing_map_.has_storage() ? ::std::optional<Storage>(routing_map_.storage()) : ::std::nullopt;
  c10::intrusive_ptr<TensorImpl> routing_map__impl_saved;
  if (routing_map_.defined()) routing_map__impl_saved = routing_map_.getIntrusivePtr();
  #endif
  auto _tmp = ([&]() {
    at::AutoDispatchBelowADInplaceOrView guard;
      return at_npu::redispatch::npu_moe_token_permute_with_routing_map(ks & c10::after_autograd_keyset, tokens_, routing_map_, probs, num_out_tokens, drop_and_pad);
  })();
  auto [result0, result1, result2] = std::move(_tmp);
  #ifndef NDEBUG
  if (tokens__storage_saved.has_value() &&
      !at::impl::dispatch_mode_enabled() &&
      !at::impl::tensor_has_dispatch(tokens_) &&
      !at::impl::tensor_has_dispatch(tokens_))
    TORCH_INTERNAL_ASSERT(tokens__storage_saved.value().is_alias_of(tokens_.storage()));
  if (tokens__impl_saved && !at::impl::dispatch_mode_enabled() && !at::impl::tensor_has_dispatch(tokens_))
    TORCH_INTERNAL_ASSERT(tokens__impl_saved == tokens_.getIntrusivePtr());
  if (routing_map__storage_saved.has_value() &&
      !at::impl::dispatch_mode_enabled() &&
      !at::impl::tensor_has_dispatch(routing_map_) &&
      !at::impl::tensor_has_dispatch(routing_map_))
    TORCH_INTERNAL_ASSERT(routing_map__storage_saved.value().is_alias_of(routing_map_.storage()));
  if (routing_map__impl_saved && !at::impl::dispatch_mode_enabled() && !at::impl::tensor_has_dispatch(routing_map_))
    TORCH_INTERNAL_ASSERT(routing_map__impl_saved == routing_map_.getIntrusivePtr());

  if (!at::impl::dispatch_mode_enabled() && !at::impl::tensor_has_dispatch(result0))
    TORCH_INTERNAL_ASSERT(result0.use_count() <= 1, "function: npu_moe_token_permute_with_routing_map");

  if (!at::impl::dispatch_mode_enabled() && !at::impl::tensor_has_dispatch(result1))
    TORCH_INTERNAL_ASSERT(result1.use_count() <= 1, "function: npu_moe_token_permute_with_routing_map");

  if (!at::impl::dispatch_mode_enabled() && !at::impl::tensor_has_dispatch(result2))
    TORCH_INTERNAL_ASSERT(result2.use_count() <= 1, "function: npu_moe_token_permute_with_routing_map");
  #endif
  if (grad_fn) {
      set_history(flatten_tensor_args( result0, result1 ), grad_fn);
  }
  throw_error_for_complex_autograd(result0, "npu_moe_token_permute_with_routing_map");
  throw_error_for_complex_autograd(result1, "npu_moe_token_permute_with_routing_map");
  if (grad_fn) {
    grad_fn->result2_ = SavedVariable(result2, true);
  }
  return std::make_tuple(std::move(result0), std::move(result1), std::move(result2));
...
at::Tensor _npu_dtype_cast(c10::DispatchKeySet ks, const at::Tensor & self, at::ScalarType dtype) {
  ...
  [[maybe_unused]] auto _any_requires_grad = compute_requires_grad( self );
  _any_requires_grad &= (isDifferentiableType(dtype));
  std::shared_ptr<NpuDtypeCastBackward1> grad_fn;
  if (_any_requires_grad) {
    grad_fn = std::shared_ptr<NpuDtypeCastBackward1>(new NpuDtypeCastBackward1(), deleteNode);
    grad_fn->set_next_edges(collect_next_edges( self ));
    grad_fn->self_scalar_type = self.scalar_type();
  }
  #ifndef NDEBUG
  ...
  auto _tmp = ([&]() {
    at::AutoDispatchBelowADInplaceOrView guard;
      return at_npu::redispatch::_npu_dtype_cast(ks & c10::after_autograd_keyset, self_, dtype);
  })();
  auto result = std::move(_tmp);
  #ifndef NDEBUG
  if (self__storage_saved.has_value() &&
  ...
  if (!at::impl::dispatch_mode_enabled() && !at::impl::tensor_has_dispatch(result))
    TORCH_INTERNAL_ASSERT(result.use_count() <= 1, "function: _npu_dtype_cast");
  #endif
  if (grad_fn) {
      set_history(flatten_tensor_args( result ), grad_fn);
  }
...
::std::tuple<at::Tensor,at::Tensor,at::Tensor,at::Tensor,int64_t,int64_t,int64_t> npu_fusion_attention(c10::DispatchKeySet ks, const at::Tensor & query, const at::Tensor & key, const at::Tensor & value, int64_t head_num, c10::string_view input_layout, const ::std::optional<at::Tensor> & pse, const ::std::optional<at::Tensor> & padding_mask, const ::std::optional<at::Tensor> & atten_mask, double scale, double keep_prob, int64_t pre_tockens, int64_t next_tockens, int64_t inner_precise, at::OptionalIntArrayRef prefix, at::OptionalIntArrayRef actual_seq_qlen, at::OptionalIntArrayRef actual_seq_kvlen, int64_t sparse_mode, bool gen_mask_parallel, bool sync, c10::string_view softmax_layout) {
  ...
  if (_any_requires_grad) {
    ...
    grad_fn->scale = scale;
    grad_fn->softmax_layout = std::string(softmax_layout);
  ...
  auto _tmp = ([&]() {
    at::AutoDispatchBelowADInplaceOrView guard;
      return at_npu::redispatch::npu_fusion_attention(ks & c10::after_autograd_keyset, query_, key_, value_, head_num, input_layout, pse, padding_mask, atten_mask, scale, keep_prob, pre_tockens, next_tockens, inner_precise, prefix, actual_seq_qlen, actual_seq_kvlen, sparse_mode, gen_mask_parallel, sync, softmax_layout);
  })();
...
at::Tensor npu_gather_sparse_index(c10::DispatchKeySet ks, const at::Tensor & input, const at::Tensor & index) {
  ...
  check_no_requires_grad(index, "index", "npu_gather_sparse_index");
  std::shared_ptr<NpuGatherSparseIndexBackward0> grad_fn;
  if (_any_requires_grad) {
    grad_fn = std::shared_ptr<NpuGatherSparseIndexBackward0>(new NpuGatherSparseIndexBackward0(), deleteNode);
    grad_fn->set_next_edges(collect_next_edges( input ));
    grad_fn->index_ = SavedVariable(index, false);
  ...
  auto _tmp = ([&]() {
    at::AutoDispatchBelowADInplaceOrView guard;
      return at_npu::redispatch::npu_gather_sparse_index(ks & c10::after_autograd_keyset, input_, index_);
  })();
  auto result = std::move(_tmp);
  #ifndef NDEBUG
  if (input__storage_saved.has_value() &&
  ...
  if (!at::impl::dispatch_mode_enabled() && !at::impl::tensor_has_dispatch(result))
    TORCH_INTERNAL_ASSERT(result.use_count() <= 1, "function: npu_gather_sparse_index");
  #endif
  if (grad_fn) {
      set_history(flatten_tensor_args( result ), grad_fn);
  }
  throw_error_for_complex_autograd(result, "npu_gather_sparse_index");
  return result;
}
...
::std::tuple<at::Tensor,at::Tensor> npu_moe_token_permute(c10::DispatchKeySet ks, const at::Tensor & tokens, const at::Tensor & indices, ::std::optional<int64_t> num_out_tokens, bool padded_mode) {
  ...
  [[maybe_unused]] auto _any_requires_grad = compute_requires_grad( tokens );

  check_no_requires_grad(indices, "indices", "npu_moe_token_permute");
  std::shared_ptr<NpuMoeTokenPermuteBackward0> grad_fn;
  if (_any_requires_grad) {
    grad_fn = std::shared_ptr<NpuMoeTokenPermuteBackward0>(new NpuMoeTokenPermuteBackward0(), deleteNode);
    grad_fn->set_next_edges(collect_next_edges( tokens ));
    grad_fn->indices_ = SavedVariable(indices, false);
    grad_fn->padded_mode = padded_mode;
    grad_fn->tokens_ = SavedVariable(tokens, false);
  }
  #ifndef NDEBUG
  auto tokens__storage_saved =
    tokens_.has_storage() ? ::std::optional<Storage>(tokens_.storage()) : ::std::nullopt;
  ...
  auto _tmp = ([&]() {
    at::AutoDispatchBelowADInplaceOrView guard;
      return at_npu::redispatch::npu_moe_token_permute(ks & c10::after_autograd_keyset, tokens_, indices_, num_out_tokens, padded_mode);
  })();
  auto [result0, result1] = std::move(_tmp);
  #ifndef NDEBUG
  if (tokens__storage_saved.has_value() &&
  ...
  if (!at::impl::dispatch_mode_enabled() && !at::impl::tensor_has_dispatch(result0))
    TORCH_INTERNAL_ASSERT(result0.use_count() <= 1, "function: npu_moe_token_permute");

  if (!at::impl::dispatch_mode_enabled() && !at::impl::tensor_has_dispatch(result1))
    TORCH_INTERNAL_ASSERT(result1.use_count() <= 1, "function: npu_moe_token_permute");
  #endif
  if (grad_fn) {
      set_history(flatten_tensor_args( result0 ), grad_fn);
  }
  throw_error_for_complex_autograd(result0, "npu_moe_token_permute");
  if (grad_fn) {
    grad_fn->result1_ = SavedVariable(result1, true);
  }
  return std::make_tuple(std::move(result0), std::move(result1));
...
at::Tensor npu_moe_token_unpermute(c10::DispatchKeySet ks, const at::Tensor & permuted_tokens, const at::Tensor & sorted_indices, const ::std::optional<at::Tensor> & probs, bool padded_mode, at::OptionalIntArrayRef restore_shape) {
  auto& permuted_tokens_ = unpack(permuted_tokens, "permuted_tokens", 0);
  auto& sorted_indices_ = unpack(sorted_indices, "sorted_indices", 1);
  [[maybe_unused]] auto _any_requires_grad = compute_requires_grad( permuted_tokens, probs );

  check_no_requires_grad(sorted_indices, "sorted_indices", "npu_moe_token_unpermute");
  std::shared_ptr<NpuMoeTokenUnpermuteBackward0> grad_fn;
  if (_any_requires_grad) {
    grad_fn = std::shared_ptr<NpuMoeTokenUnpermuteBackward0>(new NpuMoeTokenUnpermuteBackward0(), deleteNode);
    grad_fn->set_next_edges(collect_next_edges( permuted_tokens, probs ));
    grad_fn->padded_mode = padded_mode;
    grad_fn->permuted_tokens_ = SavedVariable(permuted_tokens, false);
    grad_fn->probs_ = SavedVariable(probs, false);
    grad_fn->restore_shape = restore_shape;
    grad_fn->sorted_indices_ = SavedVariable(sorted_indices, false);
  }
  #ifndef NDEBUG
  auto permuted_tokens__storage_saved =
    permuted_tokens_.has_storage() ? ::std::optional<Storage>(permuted_tokens_.storage()) : ::std::nullopt;
  c10::intrusive_ptr<TensorImpl> permuted_tokens__impl_saved;
  if (permuted_tokens_.defined()) permuted_tokens__impl_saved = permuted_tokens_.getIntrusivePtr();
  auto sorted_indices__storage_saved =
    sorted_indices_.has_storage() ? ::std::optional<Storage>(sorted_indices_.storage()) : ::std::nullopt;
  c10::intrusive_ptr<TensorImpl> sorted_indices__impl_saved;
  if (sorted_indices_.defined()) sorted_indices__impl_saved = sorted_indices_.getIntrusivePtr();
  #endif
  auto _tmp = ([&]() {
    at::AutoDispatchBelowADInplaceOrView guard;
      return at_npu::redispatch::npu_moe_token_unpermute(ks & c10::after_autograd_keyset, permuted_tokens_, sorted_indices_, probs, padded_mode, restore_shape);
  })();
  auto result = std::move(_tmp);
  #ifndef NDEBUG
  if (permuted_tokens__storage_saved.has_value() &&
      !at::impl::dispatch_mode_enabled() &&
      !at::impl::tensor_has_dispatch(permuted_tokens_) &&
      !at::impl::tensor_has_dispatch(permuted_tokens_))
    TORCH_INTERNAL_ASSERT(permuted_tokens__storage_saved.value().is_alias_of(permuted_tokens_.storage()));
  if (permuted_tokens__impl_saved && !at::impl::dispatch_mode_enabled() && !at::impl::tensor_has_dispatch(permuted_tokens_))
    TORCH_INTERNAL_ASSERT(permuted_tokens__impl_saved == permuted_tokens_.getIntrusivePtr());
  if (sorted_indices__storage_saved.has_value() &&
      !at::impl::dispatch_mode_enabled() &&
      !at::impl::tensor_has_dispatch(sorted_indices_) &&
      !at::impl::tensor_has_dispatch(sorted_indices_))
    TORCH_INTERNAL_ASSERT(sorted_indices__storage_saved.value().is_alias_of(sorted_indices_.storage()));
  if (sorted_indices__impl_saved && !at::impl::dispatch_mode_enabled() && !at::impl::tensor_has_dispatch(sorted_indices_))
    TORCH_INTERNAL_ASSERT(sorted_indices__impl_saved == sorted_indices_.getIntrusivePtr());

  if (!at::impl::dispatch_mode_enabled() && !at::impl::tensor_has_dispatch(result))
    TORCH_INTERNAL_ASSERT(result.use_count() <= 1, "function: npu_moe_token_unpermute");
  #endif
  if (grad_fn) {
      set_history(flatten_tensor_args( result ), grad_fn);
  }
  throw_error_for_complex_autograd(result, "npu_moe_token_unpermute");
  return result;
}
::std::tuple<at::Tensor,at::Tensor,at::Tensor,at::Tensor> _npu_moe_token_unpermute_with_routing_map(c10::DispatchKeySet ks, const at::Tensor & permuted_tokens, const at::Tensor & sorted_indices, at::IntArrayRef restore_shape, const ::std::optional<at::Tensor> & probs, const ::std::optional<at::Tensor> & routing_map, bool drop_and_pad) {
  ...
  check_no_requires_grad(sorted_indices, "sorted_indices", "_npu_moe_token_unpermute_with_routing_map");
  check_no_requires_grad(routing_map, "routing_map", "_npu_moe_token_unpermute_with_routing_map");
  std::shared_ptr<NpuMoeTokenUnpermuteWithRoutingMapBackward0> grad_fn;
  if (_any_requires_grad) {
    grad_fn = std::shared_ptr<NpuMoeTokenUnpermuteWithRoutingMapBackward0>(new NpuMoeTokenUnpermuteWithRoutingMapBackward0(), deleteNode);
    grad_fn->set_next_edges(collect_next_edges( permuted_tokens, probs ));
    grad_fn->drop_and_pad = drop_and_pad;
    grad_fn->permuted_tokens_ = SavedVariable(permuted_tokens, false);
    grad_fn->probs_ = SavedVariable(probs, false);
    grad_fn->restore_shape = restore_shape.vec();
    grad_fn->routing_map_ = SavedVariable(routing_map, false);
    grad_fn->sorted_indices_ = SavedVariable(sorted_indices, false);
  }
  #ifndef NDEBUG
  ...
  auto _tmp = ([&]() {
    at::AutoDispatchBelowADInplaceOrView guard;
      return at_npu::redispatch::_npu_moe_token_unpermute_with_routing_map(ks & c10::after_autograd_keyset, permuted_tokens_, sorted_indices_, restore_shape, probs, routing_map, drop_and_pad);
  })();
  auto [result0, result1, result2, result3] = std::move(_tmp);
  #ifndef NDEBUG
  if (permuted_tokens__storage_saved.has_value() &&
  ...
  if (!at::impl::dispatch_mode_enabled() && !at::impl::tensor_has_dispatch(result0))
    TORCH_INTERNAL_ASSERT(result0.use_count() <= 1, "function: _npu_moe_token_unpermute_with_routing_map");

  if (!at::impl::dispatch_mode_enabled() && !at::impl::tensor_has_dispatch(result1))
    TORCH_INTERNAL_ASSERT(result1.use_count() <= 1, "function: _npu_moe_token_unpermute_with_routing_map");

  if (!at::impl::dispatch_mode_enabled() && !at::impl::tensor_has_dispatch(result2))
    TORCH_INTERNAL_ASSERT(result2.use_count() <= 1, "function: _npu_moe_token_unpermute_with_routing_map");

  if (!at::impl::dispatch_mode_enabled() && !at::impl::tensor_has_dispatch(result3))
    TORCH_INTERNAL_ASSERT(result3.use_count() <= 1, "function: _npu_moe_token_unpermute_with_routing_map");
  #endif
  if (grad_fn) {
      set_history(flatten_tensor_args( result0 ), grad_fn);
  }
  throw_error_for_complex_autograd(result0, "_npu_moe_token_unpermute_with_routing_map");
  if (grad_fn) {
    grad_fn->result1_ = SavedVariable(result1, true);
    grad_fn->result2_ = SavedVariable(result2, true);
  }
...
TORCH_LIBRARY_IMPL(npu, AutogradPrivateUse1, m) {
    ...
    m.impl("_npu_format_cast.aclnn",
           TORCH_FN(VariableType::_npu_format_cast_aclnn)
    );
    m.impl("npu_gelu",
    ...
    m.impl("npu_moe_token_permute_with_routing_map",
           TORCH_FN(VariableType::npu_moe_token_permute_with_routing_map)
    );
    ...
    m.impl("_npu_dtype_cast",
           TORCH_FN(VariableType::_npu_dtype_cast)
    );
    ...
    m.impl("npu_gather_sparse_index",
           TORCH_FN(VariableType::npu_gather_sparse_index)
    );
    ...
    m.impl("npu_moe_token_permute",
           TORCH_FN(VariableType::npu_moe_token_permute)
    );
    m.impl("npu_moe_token_unpermute",
           TORCH_FN(VariableType::npu_moe_token_unpermute)
    );
    m.impl("_npu_moe_token_unpermute_with_routing_map",
           TORCH_FN(VariableType::_npu_moe_token_unpermute_with_routing_map)
    );
}
